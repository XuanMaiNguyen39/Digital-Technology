...
## [TAB] a dash mantine button menu to switch the column value of the graphs
choices = html.Div(
    [
...
def update_graphs(value, clicked_category, clicked_subcategory, clicked_segment, clicked_state, app_state):
   ...
   # get the values of the 'global variables' from the dcc.Store input
    ...
    category_list = [d.get('value') for d in inputs if d.get('input') == 'by_category']
    ...
    selected_category = category_list[-1] if len(category_list) >= 1 else None
